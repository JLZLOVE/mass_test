...
function p(text) {
  return new Paragraph({ spacing: { after: 60, line: 300 }, children: [new TextRun({ text, font: FONT, size: 20 })] });
}
function bp(prefix, text) {
  return new Paragraph({
    spacing: { after: 30, line: 300 }, indent: { left: 360 },
    children: [
...
function line() {
  return new Paragraph({ spacing: { before: 40, after: 40 }, border: { bottom: { style: BorderStyle.SINGLE, size: 1, color: "DDDDDD" } }, children: [] });
}
...
const doc = new Document({
  ...
  sections: [{
    ...
    children: [
      ...
      bp("后端开发：", "设计 21 张数据表，实现 20+ 个 RESTful API 接口，涵盖用户管理、RBAC 权限控制、多级审批流程、数据校验与防篡改机制。"),
      bp("数据库设计：", "使用 MySQL 进行数据库建模，对高频字段建立索引优化查询性能；使用 Redis 缓存热点数据，提升系统响应速度。"),
      bp("代码规范：", "设计统一错误码体系，集成 Swagger/Knife4j 自动生成 API 文档，通过 Git 进行版本管理，保证代码可维护性。"),
