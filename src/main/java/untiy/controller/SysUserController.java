...
public class SysUserController {
    ...
    //==============禁用用户==============================
/*    @RequiresLevel(minLevel = Level.ADMIN)
    @Operation(summary = "禁用用户", description = "管理员及以上可调用，仅能修改权限范围内用户")
    @PutMapping("/{id}/status/{status}")
    public R enableOrDisable(@PathVariable String username
                                                ,@PathVariable Integer status) {
        sysUserService.enableOrDisable(username, status);
        return R.ok();
    }*/
